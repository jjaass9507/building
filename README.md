...
- **單棟 3D 示意圖**：由現有樓層資料建立 SVG 立體量體，指標直接顯示於樓層立面，支援立體／正視與點選詳細資料。
...
每棟標題的 `3D` 按鈕與廠棟概況均可開啟。資料沿用 `/api/data`，無須另建模型。

- 採用 SVG 固定斜視角投影，以正面、頂面、側面與基座呈現實心量體；可切換正視。
- 樓層名稱與單一指標直接置於該層立面，移除獨立右側標籤列。可切換樓高、荷重、製程、面積。
- 滑鼠移入或鍵盤聚焦時整層高亮；點選、Enter 或空白鍵開啟該層附近的完整資訊卡與短連線。Escape 先關閉資訊卡，再按一次關閉視窗。
- 長文字依實際字寬截短，完整內容保留於提示、無障礙名稱與詳細資訊卡。文字維持桌面 14px、手機 13px。
- ResizeObserver 依可用畫面重新配置。一般樓層數呈現單棟完整量體；高度不足時以有範圍標題的連續樓段並排，確保桌面多樓層同屏可讀。極窄或極矮畫面仍不足時保留字級並提供捲動提示。
- 青綠表示選取，橘色虛線表示未成廠；支援深色模式。
- `ALL` 不建立虛構樓層；空資料提供提示。地下室依既有樓層排序位於底部。
- 移除自由旋轉、縮放與舊 CSS 3D 狀態，避免標籤遮擋。原生 SVG／JavaScript，沒有新增套件。

> 量體寬度僅參考面積做有限變化，樓高、間距與外形為資訊示意，不代表實際工程尺寸或 BIM 模型。
