...
- 樓層改為包含外牆、窗帶與轉角收邊的實心量體，並加入貫穿樓層的核心筒、屋頂與建築基座，降低樓板懸空感。
- 模型固定呈現完整建築量體，不提供沒有額外樓層內容可檢視的分層展開模式；可切換立體／正視。
- 每一樓層的標註直接疊加在 3D 畫布上，透過端點與短連線對準樓層；可切換樓高／荷重／製程／面積四種單一指標。
- 模型與樓層標籤會依可用高度自動縮放及等距排列，桌面版不需捲動即可查看所有樓層與目前指標。
- 選取樓層的完整資料卡也顯示於 3D 畫布內，模型、標註、對照線與資料卡使用同一視覺座標，便於快速對應。
- 所有改動皆使用原生 CSS／JavaScript，無新套件。
